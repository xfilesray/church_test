...
with st.sidebar.form(key="f_main", clear_on_submit=False):
    ...
    if sel_group == c.OTHER_CUSTOM_TRIGGER:
        final_group = st.text_input("Custom Group Name", placeholder="Enter group name...")
    else:
    ...
    if selected_type_key == "ROOMS":
        ...
        if sel_room == c.OTHER_CUSTOM_TRIGGER:
            final_room = st.text_input("Custom Room Name", placeholder="Enter room name...")
        else:
            final_room = sel_room
            
        role = "場地借用"
        content = st.text_input("Usage Summary", placeholder="e.g. Youth Choir Practice")
        people = st.text_input("Contact Person", placeholder="e.g. John Doe")
        grace_notes = st.text_area("Notes", placeholder="Equipments / Memos...", height=100)
        
    else:
        final_room = ""
        sel_role = st.selectbox("Select Role", c.ROLES)
        if sel_role == c.OTHER_CUSTOM_TRIGGER:
            role = st.text_input("Custom Role Name", placeholder="Enter role name...")
        else:
            role = sel_role
            
        content = st.text_input("Content Summary", placeholder="e.g. Sunday Worship")
        people = st.text_input("Staff Name(s)", placeholder="e.g. Mary, Peter (Comma separated)")
    ...
    if not df_raw.empty:
        if selected_type_key == "SCHEDULE":
            ...
            if not same_time_role.empty:
                ...
                conflict_msg.append(f"Conflict: Role [{role}] repeating on {date_str} {time_str}")
            
            # Staff Conflict (Proper Split Parsing)
            if people.strip():
                ...
                for name in input_names:
                    for idx, row in day_time_records.iterrows():
                        if name in row["people"]:
                            conflict_detected = True
                            conflict_msg.append(f"Conflict: Staff [{name}] is already assigned on {date_str} {time_str}")
        
        elif selected_type_key == "ROOMS":
            # Room Conflict
            same_room_time = df_raw[
                (df_raw["service_date"] == date_str) & 
                (df_raw["service_time"] == time_str) & 
                (df_raw["room_name"] == final_room) & 
                (df_raw["record_type"] == "ROOMS")
            ]
            if not same_room_time.empty:
                conflict_detected = True
                conflict_msg.append(f"Conflict: Room [{final_room}] is already booked on {date_str} {time_str}")

    if conflict_detected:
        st.session_state["c_m"] = conflict_msg
        st.session_state["p_d"] = (date_str, time_str, role, content, final_group, people, grace_notes, selected_type_key, final_room)
    else:
        save_to_supabase(date_str, time_str, role, content, final_group, people, grace_notes, selected_type_key, final_room)
...
with tab1:
    ...
    if df_raw.empty:
        ...
        st.dataframe(df_s_disp[list(c.DF_COL_MAP_SCHEDULE.values())], use_container_width=True)
...
with tab2:
    ...
    if df_raw.empty:
        ...
        st.dataframe(df_r_disp[list(c.DF_COL_MAP_ROOMS.values())], use_container_width=True)
...
with tab3:
    ...
    if df_raw.empty:
        ...
        st.dataframe(df_g_disp[list(c.DF_COL_MAP_DIARY.values())], use_container_width=True)
        st.markdown("---")
        ...
        if not df_g_disp.empty:
            ...
            if selected_key is not None:
                ...
                st.info(f"Details: {f_row['日期']} {f_row['時間']} | {f_row['崗位']} | {f_row['所屬小組']}")
